...
class TravelbooksModelHotels extends JModel
{
	/**
...
// Load the data
		...
		if (empty( $this->_hotels )) {
			$query = ' SELECT DISTINCT city FROM #__tb_hotels ';
			$this->_db->setQuery( $query );
			$this->_hotels = $this->_db->loadObjectList();
		}

		if (!$this->_hotels) {
			$this->_hotels = new stdClass();
			$this->_hotels->id = 0;
		}
...
// get the total number of records
	...
	*/
	...
	// get the subset (based on limits) of required records
		$query = 'SELECT hotels.*, cc.title AS category'
		. ' FROM #__tb_hotels AS hotels'
		. ' LEFT JOIN #__categories AS cc ON cc.id = hotels.catid'
		. $where
		. $orderby
		;
...
*/
function &changeHotels( $cid=null, $state=0 )
{
	global $mainframe;
...
//	. ' AND ( checked_out = 0 OR ( checked_out = '. (int) $user->get('id') .' ) )'
	...
	$mainframe->redirect( 'index.php?option=com_travelbook&task=hotels.show' );
}
...
function orderHotels( $uid, $inc )
{
	...
	$mainframe->redirect( 'index.php?option=com_travelbook&task=hotels.show', $msg );
 ...
 */
	...
	*/
	...
	// get the subset (based on limits) of required records
		...
		. ' FROM #__tb_jrn_htl INNER JOIN (#__categories INNER JOIN #__tb_journey ON #__categories.id = #__tb_journey.catid) ON #__tb_jrn_htl.jrn_id = #__tb_journey.id'
